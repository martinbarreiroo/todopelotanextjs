...
function Calendar({
  ...
  matches = [], // Step 1: Accept matches prop
  ...props
}) {
  // Assuming matches is an array of match objects and each has a 'date' property
  const matchDates = Array.isArray(matches)
    ? matches
  ...
  // Define a modifier for match days
  const modifiers = {
    matchDay: (date) =>
      matchDates.some(
        (matchDate) =>
  ...
  // Define custom styles for match days
  const modifiersStyles = {
    matchDay: { backgroundColor: "#729560", color: "#000" }, // Example style
  };
  return (
    <DayPicker
      ...
      modifiersStyles={modifiersStyles} // Apply custom styles for modifiers
      components={{
